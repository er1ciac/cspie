...
#define ROTL(x, n) (((x) << (n)) | ((x) >> (32 - (n)))) // roll left
// 非线性变换
static uint32_t tau(uint32_t b)
{
    return (SBox[(b >> 24) & 0xFF] << 24) | (SBox[(b >> 16) & 0xFF] << 16) | (SBox[(b >> 8) & 0xFF] << 8) | (SBox[b & 0xFF]);
}
// 线性变换
...
void sm4_encrypt_block(const uint8_t input[16], uint8_t output[16], const uint32_t rk[32])
{
    ...
    for (int i = 0; i < 32; i++)
    {
        uint32_t x = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i];
        uint32_t t = Table0[(x >> 24) & 0xFF] ^ Table1[(x >> 16) & 0xFF] ^
                     Table2[(x >> 8) & 0xFF] ^ Table3[x & 0xFF];
        X[i + 4] = X[i] ^ t;
    }
    ...
    {
        uint32_t v = X[35 - i];
        output[4 * i] = (v >> 24) & 0xFF;
        output[4 * i + 1] = (v >> 16) & 0xFF;
        output[4 * i + 2] = (v >> 8) & 0xFF;
        output[4 * i + 3] = v & 0xFF;
    }
}
